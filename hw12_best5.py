...
torch.manual_seed(0)
if not os.path.isdir('./result/resnet34'):
    os.makedirs('./result/resnet34')
...
# './result/resnet34/resnet34_domain_model.bin'))
...
for epoch in range(num_epoch):
    ...
    if train_F_loss < best_loss and best_acc < train_acc:
        ...
        torch.save(feature_extractor.state_dict(),
                   f'./result/resnet34/resnet34_extractor_model.bin')
        torch.save(label_predictor.state_dict(),
                   f'./result/resnet34/resnet34_predictor_model.bin')
        torch.save(domain_classifier.state_dict(),
                   f'./result/resnet34/resnet34_domain_model.bin')
...
plt.xlabel('epoch')
plt.ylabel('Loss')
plt.savefig('./result/resnet34/feature_extractor_loss')
plt.close()
...
plt.xlabel('epoch')
plt.ylabel('Loss')
plt.savefig('./result/resnet34/Domain_discriminator_loss')
plt.close()
...
df.to_csv('./result/resnet34/feature_extractor_loss.csv')

plt.plot(acc, 'b-')
plt.title('Accuracy')
plt.xlabel('epoch')
plt.ylabel('Accuracy')
plt.savefig('./result/resnet34/Accuracy')
plt.close()
"""# Inference
